...
public class DBFilms {
    ...
    public DBFilms(Context context) {
        OpenHelper mOpenHelper = new OpenHelper(context);
        fDataBase = mOpenHelper.getWritableDatabase();
    }
    ...
    public long insert(String name, String author, String comment, String category, int review, int rang) {
        ...
        return fDataBase.insert(TABLE_NAME, null, cv);
    }
    ...
    return fDataBase.update(TABLE_NAME, cv, COLUMN_ID + " = ?",new String[] { String.valueOf(vf.getId())});
    }

    public void deleteAll() {
        fDataBase.delete(TABLE_NAME, null, null);
    }

    public void delete(long id) {
        fDataBase.delete(TABLE_NAME, COLUMN_ID + " = ?", new String[] { String.valueOf(id) });
    }

    public ClassFilms select(long id) {
        Cursor mCursor = fDataBase.query(TABLE_NAME, null, COLUMN_ID + " = ?", new String[]{String.valueOf(id)}, null, null, null);
    ...
    public ArrayList<String> selectCategories(){
        Cursor userCursor =  fDataBase.rawQuery("SELECT * FROM `" + TABLE_NAME
                + "` WHERE `Rang` = 0 ORDER BY `Category`", null);
